...
"""
    ...
    def pbias(y_obs,y_mod):     percent bias
...
def pbias(y_obs,y_mod):
    """
    calculates bias = 100.0 * sum(y_mod - y_obs) / sum (y_obs) 
    ...
    >>> y_mod = np.array([12.8087, 13.151, 14.3741, 16.2302, 17.9433])
    >>> # calculate pbias
    >>> print(np.round(pbias(y_obs, y_mod),2))
    2.95
    ...
    #     y_obsr = np.ma.array(y_obs, mask=y_mod.mask | y_obs.mask)
    # return np.ma.mean(y_obsr) - np.ma.mean(y_modr)
    return 100.0 * (y_mod - y_obs).sum() / y_obs.sum()
