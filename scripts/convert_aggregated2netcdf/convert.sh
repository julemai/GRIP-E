...
for grid in ${grids} ; do
	...
	if [[ ${grid} == 'RDRS-v2' ]] ; then
	    inputfile="${datapath}landcover_NALCMS_GreatLakes/landcover_NALCMS_GreatLakes_aggregated_v1.1/GL_NALCMS_landcover_rdrs_v2.txt"
	    gridfile="${datapath}meteo_forcing_RDRS-v2/grip-gl_rdrs-v2-gridonly.nc"
	    #
	    ...
	    legendsfile=""  
	    varinfofile="${datapath}landcover_NALCMS_GreatLakes/landcover_NALCMS_GreatLakes_aggregated_v1.1/variable_info.csv"
	else
	    if [[ ${grid} == 'WFDEI-GEM-CaPA' ]] ; then
		inputfile="${datapath}landcover_NALCMS_GreatLakes/landcover_NALCMS_GreatLakes_aggregated_v1.1/GL_NALCMS_landcover_wfdei_gem_capa.txt"
		gridfile="${datapath}meteo_forcings_WFDEI-GEM-CaPA/grip-gl_wfdei-gem-capa_gridonly.nc"
		#
		# # stores variables 4D (time, lat, lon, n_LC_types)
		# legendsfile="${datapath}landcover_NALCMS_GreatLakes/NACLMS_legends.csv"
